...
// Добавление строк в таблицу и присвоение id
...
window.addEventListener("click", (event) => {
    ...
    if (modal && !modal.contains(event.target) && !event.target.classList.contains("copy-btn")) {
        modal.classList.add("hidden");
        modal.removeAttribute("id"); // Удаляем постоянный id для перезаписи
    }
});
